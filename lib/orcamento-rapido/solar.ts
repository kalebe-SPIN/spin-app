...
export type EntradaSolar =
  | { modo: 'consumo_kwh'; consumo_kwh: number }
  | { modo: 'qtd_placas'; qtd_placas: number }
  | { modo: 'valor_mensal'; valor_mensal: number }
  | { modo: 'fatura'; fatura_url?: string; consumo_extraido?: number }
 ...
 */
function kwpDeConsumo(consumo_kwh_mes: number, params: ParametrosOrcamento): number {
  const geracao_kwh_por_kwp_mes = params.fator_dimensionamento_sc * 30 * 0.85
...
export const adaptadorSolar: AdaptadorOrcamento<EntradaSolar> = {
  ...
  calcular(entrada, params) {
    ...
    if (entrada.modo === 'consumo_kwh') {
      kwp = kwpDeConsumo(entrada.consumo_kwh, params)
      qtd_placas = qtdPlacasDeKwp(kwp, params)
      baseHint = `Baseado em ${entrada.consumo_kwh} kWh/mês`
      ...
      kwp = kwpDeConsumo(consumo, params)
      qtd_placas = qtdPlacasDeKwp(kwp, params)
      baseHint = `Baseado em R$ ${fmtBRL(entrada.valor_mensal)}/mês (≈ ${consumo.toFixed(0)} kWh)`
    } else if (entrada.modo === 'fatura' && entrada.consumo_extraido) {
      kwp = kwpDeConsumo(entrada.consumo_extraido, params)
      qtd_placas = qtdPlacasDeKwp(kwp, params)
    ...
    // (garante que estimativa bate com kit sugerido)
    const kwpReal = kwpDeQtdPlacas(qtd_placas, params)
    const valor_estimado = Math.round(kwpReal * params.preco_medio_kwp_instalado)
    const kit = escolherKitSugerido(kwpReal, params)

    return {
      valor_estimado,
      resumo: `Sistema solar ${kwpReal.toFixed(2).replace('.', ',')} kWp · ${qtd_placas} placas`,
      detalhes: [
        { label: 'Potência estimada', valor: `${kwpReal.toFixed(2).replace('.', ',')} kWp` },
        { label: 'Quantidade de placas', valor: `${qtd_placas} × ${params.potencia_padrao_modulo_wp}Wp` },
        { label: 'Kit sugerido', valor: kit },
        { label: 'Estimativa', valor: baseHint },
      ],
      estimativa_tecnica: {
        kwp: kwpReal,
        qtd_placas,
      ...
      observacao_padrao: '*Valor sujeito a análise técnica no local. Proposta oficial após visita.*',
    }
  },

  formatarWhatsApp(entrada, resultado, empresa, valorFinal) {
    const tec = resultado.estimativa_tecnica as { kwp: number; qtd_placas: number; kit_sugerido: string }
